          ...
          <tr v-for="(goods, index) in order.orderDetailList" :key="goods.id">
            <td width="60%">
              <div class="typographic">
                ...
                <span>x{{ goods.skuNum }}--{{ goods.orderPrice }}</span>
            ...
            <template v-if="index === 0">
              ...
              >
                <a href="#" class="btn">{{
                  order.processStatus === 'UNPAID' ? '未支付' : '已完成'
                }}</a>
              </td>
              <td
                :rowspan="order.orderDetailList.length"
              ...
              >
                <ul class="unstyled">
                  <li>
                    <a href="mycomment.html">{{ order.orderComment }}|晒单</a>
                  </li>
                </ul>
              </td>
            </template>
...
export default {
  ...
  async mounted() {
    //获取数据列表,只用于获取数据,不能重复使用,所以可以放在methods中
    // const result = await reqGetOrderList(this.page, this.limit);
    //console.log(result);
  ...
  methods: {
    ...
    async getOrders(page = 1) {
      this.page = page;
      const result = await reqGetOrderList(page, this.limit);
      // console.log(result);
      const { records, total } = result;
